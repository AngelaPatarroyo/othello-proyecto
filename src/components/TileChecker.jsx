...
/* tIf it's black's turn, change the turn to white's */
        if (turn === false) {
            setTurn(true);
            setEndGame(false);
        }
        if (newMatriz[x][y] === empty) {
/* Check if the tile where the token is to be placed is empty*/
...
/* If it finds a token of the same color at the end of the direction, it can flip the tokens in that directiond*/
...
/* If tokens should be flipped, update the matrix and change the turn */
            if (shouldFlip) {
                ...
                setTurn(!turn);
                return true;
/* Indicate that the move was valid */
...
/* If tokens cannot be flipped in any direction, show an alert and return false */
                ...
                return false;
/* Indicate that the move was not allowed */
            }
        }
        return false;
/* If the tile is not empty, return false to disallow the player from placing a token on 
